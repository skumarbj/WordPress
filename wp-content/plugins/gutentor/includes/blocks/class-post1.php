...
if ( ! class_exists( 'Gutentor_Post1' ) ) {
	...
	class Gutentor_Post1 extends Gutentor_Block_Base{
		...
		public function load_dependencies(){
			...
			require_once GUTENTOR_PATH . 'includes/block-templates/class-post1-templates.php';
		}
		...
		public function render_callback( $attributes, $content ) {
			...
			$template = $attributes['post1Temp'] ? $attributes['post1Temp'] : '';
			...
			if ($the_query->have_posts()) :
				$output  .= '<'.$tag.' class="'.apply_filters('gutentor_post_module_main_wrap_class',gutentor_concat_space('gutentor-post-module','gutentor-post-module-post1', 'section-'.$gID,$template,$align), $attributes).'" id="'. esc_attr($blockID) . '" '.GutentorAnimationOptionsDataAttr($blockComponentAnimation).'>' . "\n";
				...
				while ($the_query->have_posts()) : $the_query->the_post();
					...
					$output .= apply_filters('gutentor_post_module_post1_template_data', '',get_post(), $attributes);
...
Gutentor_Post1::get_instance()->run();