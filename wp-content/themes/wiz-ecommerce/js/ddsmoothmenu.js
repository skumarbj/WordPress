	...
	var flattened = flattenuls($mainul, false,
		function(i, $header, $subul){ // loop through header LIs and sub ULs
			...
			var $breadcrumb = $('<li class="breadcrumb" />')
				.html('<span class="fa fa-arrow-left wiz-arrow-left"> </span>' + $header.text())
				.prependTo($subul)
			$header.find('a:eq(0)').append('<span class="fa fa-arrow-right wiz-arrow"></span>')
...
//calculate this sub menu's offsets from its parent
					...
					if (setting.shadow){
						if(!$curobj.data('$shadow')){
							$curobj.data('$shadow', $('<div></div>').addClass('ddshadow toplevelshadow').prependTo(
							shadow).css({zIndex: $curobj.css('zIndex')}));  //insert shadow DIV and set it to parent node for the next shadow div
						}
